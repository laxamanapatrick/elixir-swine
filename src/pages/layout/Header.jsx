...
import { Box, Button, Flex, HStack, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Text, useDisclosure } from '@chakra-ui/react'
...
export const Header = ({ setSidebarHandler }) => {
    ...
    return (
        <Flex bgColor='myBlack' w='full' p={1} justifyContent='space-between' alignItems='center' boxShadow='0px 3px 10px 0px rgba(40,40,43,1)'>
            <AiOutlineMenuUnfold color='#e5e5e5' fontSize='21px' cursor='pointer' onClick={() => setSidebarHandler(prev => !prev)} />
            <Menu>
                {({ isOpen }) => (
                    <>
                        <MenuButton w='45px' h='27px' p={1} bg='primary' _hover={{ bg: 'primary' }} _active={{ bg: 'primary' }} borderRadius='17px' as={Button} rightIcon={!isOpen && <MotionTag><AiFillCaretDown color='#e5e5e5' /></MotionTag>}>
                            {
